...
public class EmployeesController : ControllerBase
{
    ...
    public async Task<IActionResult> PartiallyUpdateEmployeeForCompany(Guid companyId, Guid id, [FromBody] JsonPatchDocument<EmployeeForUpdateDto> patchDoc)
    {
        ...
        var result = await _service.EmployeeService.GetEmployeeForPatchAsync(companyId, id, compTrackChages: false, empTrackChanges:true);

        patchDoc.ApplyTo(result.employeeToPatch, ModelState);

        // Here we prevent any invalid employeeEntity from being saved to the database.
        TryValidateModel(result.employeeToPatch);

        // Here we validate patchDoc
        if (!ModelState.IsValid)
        {
            return UnprocessableEntity(ModelState);
        }
